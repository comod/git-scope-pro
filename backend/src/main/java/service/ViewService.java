...
public class ViewService implements Disposable {
    ...
    public void dispose() {
        ...
        // Remove all model listeners to break references
        for (Map.Entry<MyModel, Consumer<MyModel.field>> entry : modelListeners.entrySet()) {
    ...
    public void initLater() {
        ...
        // Select the saved tab and activate — wrapped in flag to prevent listener from double-firing
        tabInitializationInProgress.set(true);
        try {
    ...
    private void subscribeToObservable(MyModel model) {
        Consumer<MyModel.field> listener = field -> {
            switch (field) {
                case targetBranch -> {
        ...
        modelListeners.put(model, listener);
    }
    ...
    public boolean isTabInitializationInProgress() {
        return tabInitializationInProgress.get();
    }
    ...
    private void updateScopeDisplayName() {
        GutterDataService gds = project.getService(GutterDataService.class);
        if (gds != null) {
            MyModel current = getCurrent();
            gds.setScopeDisplayName(current != null ? current.getDisplayName() : "");
        }
    }
}