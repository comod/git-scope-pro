...
internal class ScopeGutterPopupPanel(
    ...
    private var suppressCaretCancel = false
    ...
    fun show(editor: EditorEx, range: Range, e: MouseEvent) {
        ...
        val actionGroup = DefaultActionGroup().apply {
            ...
            val scopeName = try {
                project.getService(service.GutterDataService::class.java)?.scopeDisplayName ?: ""
            } catch (_: Exception) { "" }
        ...
        val wheelListener = java.awt.event.MouseWheelListener { activePopup?.cancel() }
        scrollPane.addMouseWheelListener(wheelListener)
        Disposer.register(popupDisposable) { scrollPane.removeMouseWheelListener(wheelListener) }
        editor.caretModel.addCaretListener(object : com.intellij.openapi.editor.event.CaretListener {
            override fun caretPositionChanged(event: com.intellij.openapi.editor.event.CaretEvent) {
                if (!suppressCaretCancel) activePopup?.cancel()
            }
        }, popupDisposable)
